...
export default function Upcoming({ tasks }) {
    ...
    const end = new Date(endOfWeek(today, { weekStartsOn: 1 }))
    //if less than friday add 2
    if (isSaturday(start)) {
        start.setDate(start.getDate() + 1)
    } else if (isSunday(start)) {
        start.setDate(start.getDate())
    } else {
        start.setDate(start.getDate() + 2)
    }
    //if its saturday add 1
    //if its sunday dont add any?
    ...
    return (
        <Box width="100%">
            ...
            {!isSaturday(new Date(today)) && !isSunday(new Date(today)) && (
                <Box width="100%">
